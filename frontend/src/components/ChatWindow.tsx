  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden">
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-6">
        ...
        {error ? <p className="text-[19px] text-red-600">{error}</p> : null}
      </div>

      <form
        onSubmit={(event) => void handleSubmit(event)}
        className="shrink-0 border-t border-slate-200 bg-white px-6 py-4"
      >
        <div className="mx-auto w-full max-w-3xl">
            ...
            {EMBED_OPTIONS.map((option) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-xl border px-5 py-3 text-left transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${
                    selected
                ...
                >
                  ...
                  <span
                    className={`mt-0.5 block text-base ${selected ? 'text-blue-800/80' : 'text-slate-600'}`}
                  >
                    {option.description}
                  </span>
                </button>
          ...
          <div className="flex gap-3">
            <textarea
              ...
              rows={2}
              disabled={loading}
              className="min-h-[52px] flex-1 resize-none rounded-xl border border-slate-300 px-4 py-3 text-[19px] outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 disabled:bg-slate-100"
            />
            <button
              type="submit"
